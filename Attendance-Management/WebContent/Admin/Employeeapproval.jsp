...
PreparedStatement pst=c.prepareStatement("select * from employee where Status=?");
pst.setString(1,"N");
...
out.println("<tr><th>EmployeeId</th><th>EmployeeName</th><th>Department</th><th>Status</th></tr></thead>");
out.println("<tbody border-radius=10px>");
while(rs.next())
{
out.println("<tr><td>"+rs.getInt(1)+"</td><td>"+rs.getString(2)+"</td><td>"+rs.getString(10)+"</td><td>"+rs.getString(11)+"</td></tr>");
}		
...
<% 
out.println("<form method=get action=EmployeeStatusModify.jsp>");
%>
<input type=text name=id style=font-size:14pt pattern=[0-9]{3}  /><br><br>
<input type=radio value=approve name=choice style=font-size:14pt required />Approve
<input type=radio value=Reject name=choice style=font-size:14pt required />Reject
